# راهنمای جامع فرآیند محاسبه ساعت موظفی پرسنل در سامانه شیفت‌یار

این سند به عنوان مرجع کامل فنی و کاربردی، فرآیند و الگوریتم‌های محاسبه **ساعت موظفی ماهانه پرسنل** را بر مبنای «قانون ارتقای بهره‌وری کارکنان بالینی نظام سلامت» و «قوانین عمومی خدمات کشوری / کار» در سامانه شیفت‌یار تشریح می‌کند.

---

## ۱. نمای کلی و فلسفه محاسبات

در سامانه شیفت‌یار، ساعت موظفی مشخص‌کننده حداقل زمانی است که یک پرسنل باید در طول ماه کاری در بیمارستان خدمت کند. محاسبات موظفی مستقیماً وارد موتورهای بهینه‌سازی شیفت‌بندی (`Simulated Annealing`, `OR-Tools`, `Hybrid`) شده و وضعیت شیفت‌های موظفی، اضافه‌کار و عدم تحقق ساعت موظفی (Shortfall) را کنترل می‌کند.

پرسنل در سیستم به دو گروه اصلی تقسیم می‌شوند:
1. **گروه اول (مشمول قانون ارتقای بهره‌وری):** کادر بالینی (پرستاران، بهیاران، ماماها، اتاق عمل، بیهوشی و ...) که مشمول کسورات سابقه، سختی کار و نوبت‌کاری و همچنین ضریب ۱٫۵ شب و تعطیل می‌شوند.
2. **گروه دوم (غیرمشمول / عادی):** پرسنل اداری، پشتیبانی یا نیروهای تحت قانون مدیریت خدمات کشوری/قانون کار که کسورات بهره‌وری ندارند و موظفی آن‌ها صرفاً ضرب روزهای کاری در ساعت موظف روزانه است.

همچنین امکان **تعیین دستی موظفی (Override)** برای هر پرسنل به صورت جداگانه پیش‌بینی شده است.

---

## ۲. فرمول‌ها و منطق محاسباتی

### ۲.۱. مبنای تقویمی ماه و ساعت کار روزانه
- **ساعت موظف روزانه پایه:** $\frac{22}{3} \approx 7.333333$ ساعت (معادل **۷ ساعت و ۲۰ دقیقه** به ازای هر روز کاری طبق قانون کار و خدمات کشوری).
- **روزهای کاری غیرتعطیل ماه:**
  $$\text{WorkingDays} = \text{TotalDays} - (\text{FridaysCount} + \text{OfficialHolidaysCount})$$
  - $\text{TotalDays}$: تعداد کل روزهای ماه (مثلاً ۳۰، ۳۱ یا ۲۹ روز).
  - $\text{FridaysCount}$: تعداد جمعه‌های ماه.
  - $\text{OfficialHolidaysCount}$: تعداد روزهای تعطیل رسمی تقویمی به جز جمعه‌ها (تعطیلات رسمی که روی جمعه می‌افتند نباید دوباره کسر شوند).
  - *عدم کسر پنج‌شنبه‌ها:* بیمارستان‌ها و مراکز درمانی شبانه‌روزی هستند؛ لذا روزهای پنج‌شنبه به طور پیش‌فرض روز کاری محسوب می‌شوند (`ExcludeThursdays = false`).
- **ساعت کار پایه ناخالص ماهانه (Base Monthly Hours):**
  $$\text{BaseMonthlyHours} = \text{WorkingDays} \times \frac{22}{3}$$
- **سقف‌گذاری اختیاری کارگزینی بیمارستان (Cap Base Hours to Standard Month):**
  - در تقویم کاری اداری/کارگزینی، یک ماه استاندارد بر مبنای ۴ هفته کاری ($4 \times 44 = 176$ ساعت پایه و $4 \times 6 = 24$ روز کاری موظف) تعریف می‌شود.
  - در سامانه شیفت‌یار پارامتر `CapBaseHoursToStandardMonth` (پیش‌فرض `false`) قرار دارد.
  - در صورت فعال‌سازی (`CapBaseHoursToStandardMonth = true`):
    $$\text{WorkingDays} = \min\Big(24\, ,\; \text{RawWorkingDays}\Big)$$
    $$\text{BaseMonthlyHours} = \min\Big(176.0\, ,\; \text{WorkingDays} \times \frac{22}{3}\Big)$$
  - در حالت پیش‌فرض (`false`)، روزهای کاری و ساعت پایه مستقیماً از روی روزهای کاری تقویمی محاسبه می‌شوند (مثلاً در یک ماه ۳۰ روزه با ۲۵ روز کاری: $25 \times 7.3333 = 183.33$ ساعت).

---

### ۲.۲. گروه اول: پرسنل مشمول قانون ارتقای بهره‌وری

برای پرسنل مشمول، ساعت موظفی از طریق اعمال تخفیف‌های سه‌گانه هفتگی و تبدیل آن به کسر ماهانه محاسبه می‌شود:

#### ۱. کسورات هفتگی بهره‌وری (Weekly Reductions)
مجموع کاهش هفتگی از ۳ مؤلفه تشکیل می‌شود:

$$\text{WeeklyReduction} = \min\Big(8.0\, ,\; \text{SeniorityReduction} + \text{HardshipReduction} + \text{ShiftPatternReduction}\Big)$$

> **قانون سقف کاهش هفتگی:** مجموع کسورات این سه عامل در هر هفته **نمی‌تواند از ۸ ساعت تجاوز کند**.

#### ۲. تبدیل کاهش هفتگی به کاهش ماهانه تقویمی
ضریب تبدیل هفتگی به ماهانه بر مبنای نسبت تعداد روزهای ماه به ۷ روز هفته محاسبه می‌شود:
$$\text{WeeksInMonth} = \frac{\text{TotalDays}}{7}$$
$$\text{MonthlyReductionFromWeekly} = \left(\frac{\text{TotalDays}}{7}\right) \times \text{WeeklyReduction}$$

- برای ماه ۳۱ روزه: ضریب برابر $\frac{31}{7} \approx 4.4286$ به ازای هر ۱ ساعت تخفیف هفتگی است.
- برای ماه ۳۰ روزه: ضریب برابر $\frac{30}{7} \approx 4.2857$ به ازای هر ۱ ساعت تخفیف هفتگی است.
- برای ماه ۲۹ روزه (اسفند): ضریب برابر $\frac{29}{7} \approx 4.1429$ به ازای هر ۱ ساعت تخفیف هفتگی است.

#### ۳. ساعت موظفی خالص نهایی ماهانه (Final Monthly Required Hours)
$$\text{FinalMonthlyRequiredHours} = \max\Big(0\, ,\; \text{BaseMonthlyHours} - \text{MonthlyReductionFromWeekly}\Big)$$

همچنین فیلد گردشده `FinalMonthlyRequiredHoursRounded` (گردشده به نزدیک‌ترین عدد صحیح) برای سهولت گزارش‌گیری و نمایش در دسترس است.

> **نکته حیاتی و تفکیک معماری: اعتبار ضریب ۱٫۵ شیفت‌های شب و تعطیل (Night/Holiday Multiplier)**
> طبق نص صریح قانون ارتقای بهره‌وری، ضریب ۱٫۵ برابر شیفت‌های شب و ایام تعطیل، پاداش عملکردی برای **«کارکرد مؤثر پرسنل»** (`ProductivityWorkedHoursCalculator`) در مرحله انتساب شیفت‌ها است، **نه کسر پیش‌دستانه از سقف موظفی ماهانه**.
> - **ساعت موظفی (Target):** تعهد کاری ماهیانه پرسنل است که بر اساس تقویم و کسورات سه‌گانه تعیین می‌شود.
> - **کارکرد مؤثر (Worked Hours):** هر ۱ ساعت حضور در شب یا روز تعطیل معادل ۱٫۵ ساعت کارکرد ثبت می‌شود؛ این امر باعث می‌شود پرسنل دارای شیفت‌های شب/تعطیل زودتر به ساعت موظفی خود برسند.
> - بنابراین مقدار `NightHolidayCredit` نباید از ساعت موظفی کسر گردد (کسر آن باعث افت غیرواقعی موظفی پایه تا ۱۵ ساعت می‌شد).

---

### ۲.۳. گروه دوم: پرسنل غیرمشمول (عادی / خدمات کشوری)

برای پرسنل غیرمشمول (`IsIncludedInProductivityPlan = false`):
- کسورات سابقه خدمت: ۰ ساعت.
- کسورات سختی کار: ۰ ساعت.
- کسورات الگوی نوبت‌کاری: ۰ ساعت.
- ضریب ساعات شب/تعطیل: ۱٫۰ (بدون اعتبار ۰٫۵ مازاد).
- **ساعت موظفی نهایی ماهانه:**
  $$\text{FinalMonthlyRequiredHours} = \text{BaseMonthlyHours} = \text{WorkingDays} \times \frac{22}{3}$$

---

## ۳. جداول و بازه‌های تخفیف بر اساس دستورالعمل اجرایی وزارت بهداشت

پایه ساعت کار موظف هفتگی **۴۴ ساعت** و حداکثر سقف تقلیل در هر هفته **۸ ساعت** است. کاهش هفتگی ناشی از جمع ۳ مؤلفه زیر است:

### ۳.۱. جدول کاهش بر اساس سابقه خدمت (Seniority Reduction)

| بازه سابقه بالینی / سنوات | میزان کاهش در هفته | توضیحات |
|:-------------------------:|:-------------------:|:---------|
| **۰ تا ۴ سال** | **۱٫۰ ساعت** | کلیه پرسنل بدو خدمت و طرحی نیز مشمول این بازه هستند (هرگز صفر محاسبه نمی‌شود) |
| **۴ سال و ۱ ماه تا ۸ سال** | **۲٫۰ ساعت** | |
| **۸ سال و ۱ ماه تا ۱۲ سال** | **۳٫۰ ساعت** | |
| **۱۲ سال و ۱ ماه تا ۱۶ سال** | **۴٫۰ ساعت** | |
| **۱۶ سال و ۱ ماه به بالا** | **۵٫۰ ساعت** | سقف کاهش سابقه خدمت |

> **نکته:** برای تاریخ‌های استخدام دقیق، کسر سال بر اساس ماه محاسبه می‌شود (مثلاً ۴ سال و ۱ ماه معادل ۴٫۰۸۳۳ سال و مشمول ۲٫۰ ساعت است).

### ۳.۲. جدول کاهش صعوبت و سختی کار (Hardship Reduction - حداکثر ۲٫۰ ساعت در هفته)

سامانه از دو روش ورودی **درصد** یا **امتیاز** با قاعده اعتبارسنجی انحصاری متقابل (Mutually Exclusive / XOR) پشتیبانی می‌کند:

| درصد نظام هماهنگ (`HardshipPercentage`) | امتیاز مدیریت خدمات کشوری (`HardshipScore`) | میزان کاهش در هفته |
|:---------------------------------------:|:-------------------------------------------:|:-------------------:|
| کمتر از ۸٪ یا بدون مقدار | کمتر از ۰ یا بدون مقدار | **۰٫۰ ساعت** |
| **۸ تا ۲۵ درصد** | **۰ تا ۳۷۵ امتیاز** | **۰٫۵ ساعت** |
| **۲۶ تا ۵۰ درصد** | **۳۷۶ تا ۷۵۰ امتیاز** | **۱٫۰ ساعت** |
| **۵۱ تا ۷۵ درصد** | **۷۵۱ تا ۱۰۰۰ امتیاز** | **۱٫۵ ساعت** |
| **۷۶ تا ۱۰۰ درصد** | **۱۰۰۰ امتیاز به بالا** | **۲٫۰ ساعت** |

#### اعتبارسنجی انحصاری متقابل (Mutually Exclusive / XOR Validation):
- کاربر در پرونده یا درخواست ویرایش مجاز است **صرفاً یکی از دو فیلد درصد یا امتیاز** را پر کند.
- در صورت ورود همزمان هر دو فیلد با مقادیر معتبر، سیستم خطای ولیدیشن بازمی‌گرداند.

#### استثنای رده‌های مدیریت بالینی (ماده ۴ دستورالعمل وزارت بهداشت):
- کلیه **سوپروایزرها (Supervisor)**، **سرپرستاران (Head Nurse)**، **مترون‌ها (Metron)** و **مدیران خدمات پرستاری**:
  - به صورت خودکار مشمول حداکثر سقف کاهش صعوبت کار (**۲٫۰ ساعت در هفته**) می‌شوند؛ صرف‌نظر از اینکه چه درصد یا امتیازی در پرونده آنها ثبت شده باشد.
  - تشخیص رده‌های مدیریتی از طریق فلگ‌های بولین (`IsSupervisor`، `IsHeadNurse`) یا بررسی عناوین شغلی (`Position`، `JobTitle`، `Role`) به صورت هوشمند و مستقل از حروف کوچک/بزرگ و با پشتیبانی عبارات فارسی و انگلیسی صورت می‌گیرد.

### ۳.۳. جدول کاهش نوبت‌کاری غیرمتعارف / گردشی (Shift Pattern Reduction)

طبق نص صریح دستورالعمل اجرایی وزارت بهداشت:
- **کلیه پرسنل دارای نوبت‌کاری در گردش (Rotating / TwoShift / ThreeShift):** دقیقاً **۱٫۰ ساعت کسر در هفته**.
  - هرگونه پیش‌شرط سابقه خدمت (مانند شرط قدیمی ۱۰ سال) به طور کامل حذف گردیده است و این کسر به تمام پرسنل گردشی با هر میزان سابقه تعلق می‌گیرد.
- **پرسنل با نوبت‌کاری ثابت شب:** **۱٫۰ ساعت کسر در هفته**.
- **پرسنل روزکار ثابت:** **۰٫۰ ساعت**.

---

### ۳.۴. تجمیع و سقف نهایی

$$\text{TotalWeeklyReduction} = \min\Big(8.0\, ,\; \text{Seniority} + \text{Hardship} + \text{ShiftPattern}\Big)$$
$$\text{WeeklyRequiredHours} = 44.0 - \text{TotalWeeklyReduction}$$

---

## ۴. بازنویسی دستی ساعت موظفی (Manual Override)

در موجودیت کاربر (`User`) و فرم‌های پرسنل، فیلد اختیاری زیر قرار دارد:
- **`MaxProductivityRequiredHours` (اعشاری)**

### منطق تصمیم‌گیری (`ProductivityRequiredHoursResolver`):
```mermaid
graph TD
    Start([بررسی کاربر]) --> CheckOverride{آیا MaxProductivityRequiredHours > 0 است؟}
    CheckOverride -- بله --> SetManual[اعمال مستقیم عدد دستی به عنوان ساعت موظفی]
    SetManual --> End([پایان - لغو محاسبه خودکار])
    CheckOverride -- خیر / null --> RunAuto[محاسبه خودکار بر اساس تقویم، سابقه، سختی، الگوی شیفت و شب/تعطیل]
    RunAuto --> End
```

| مقدار فیلد | رفتار سیستم |
|:-----------|:-------------|
| `null` یا `0` | ساعت موظفی به صورت خودکار با فرمول‌های بالا محاسبه می‌شود. |
| عدد مثبت (مثلاً `140`) | محاسبه خودکار نادیده گرفته شده و **دقیقاً عدد وارد شده** به عنوان ساعت موظفی ماهانه پرسنل در الگوریتم زمان‌بندی اعمال می‌شود. |

---

## ۵. اثر ساعت موظفی در زمان‌بندی و تخصیص شیفت‌ها

ساعت موظفی محاسبه‌شده (`ProductivityRequiredHours`) به عنوان پایه محاسبات موتورهای شیفت‌بندی عمل می‌کند:

### ۵.۱. تفکیک پرسنل طرحی و غیرطرحی (`ProjectPersonnelProductivityPriority`)
1. **پرسنل طرحی (`IsProjectPersonnel = true`):**
   - سقف تخصیص شیفت برای آن‌ها **دقیقاً برابر ساعت موظفی** است.
   - به هیچ عنوان اضافه کار نمی‌گیرند ($\text{MaxAllowedHours} = \text{ProductivityRequiredHours}$).
   - شیفت‌های مازاد به پرسنل غیرطرحی اختصاص می‌یابد.
2. **پرسنل غیرطرحی / رسمی / پیمانی (`IsProjectPersonnel = false` یا `null`):**
   - اولویت پر شدن موظفی با پرسنل غیرطرحی است.
   - در صورت نیاز و وجود رضایت اضافه‌کار (`OvertimeConsent = true`)، تا سقف ماهانه مجاز (پیش‌فرض ۸۰ ساعت) اضافه‌کار دریافت می‌کنند:
     $$\text{MaxAllowedHours} = \text{ProductivityRequiredHours} + \text{MaxMonthlyOvertimeHours}$$

### ۵.۲. شاخص‌های خروجی زمان‌بندی در آمار بهینه‌سازی
در خروجی اکشن‌های شیفت‌بندی (مانند `optimize-and-save`) متغیرهای زیر برای رصد موظفی تولید می‌شوند:
- `productivityRequiredHoursByUser`: هدف و کف موظفی ماهانه پرسنل.
- `productivityShortfallByUser`: میزان کسری ساعت کارکرد نسبت به موظفی (کاربر کمتر از موظفی کار کرده).
- `productivityTargetFulfillmentRate`: درصد پرسنلی که به حداقل موظفی خود رسیده‌اند.

---

## ۶. ساختار لایه‌ها و کلاس‌های کلیدی در سورس‌کد

| لایه | کلاس / مسیر | شرح وظیفه |
|:-----|:------------|:----------|
| **Domain** | `ShiftYar.Domain/Entities/ProductivityModel/ProductivityRuleConfig.cs` | نگهداری ثوابت آیین‌نامه (۴۴ ساعت، سقف ۸ ساعت، ضرایب پیش‌فرض و بازه‌ها). |
| **Domain** | `ShiftYar.Domain/Entities/ProductivityModel/StaffEmploymentInfo.cs` | انطباق داده‌های استخدامی پرسنل از موجودیت `User`. |
| **Application** | `ShiftYar.Application/Common/Utilities/HardshipRulesValidator.cs` | اعتبارسنجی انحصاری متقابل (Mutually Exclusive / XOR) درصد و امتیاز سختی کار. |
| **Application** | `ShiftYar.Application/Common/Utilities/ClinicalManagementRoleDetector.cs` | تشخیص رده‌های مدیریت بالینی (ماده ۴ دستورالعمل وزارت بهداشت: سوپروایزر، سرپرستار، مترون و مدیر پرستاری) جهت اعطای خودکار ۲.۰ ساعت کسر صعوبت کار. |
| **Application** | `ShiftYar.Application/DTOs/ProductivityModel/WorkingHoursCalculationRequestDto.cs` | مدل ورودی محاسبات شامل تقویم ماه، جمعه‌ها، تعطیلات و وضعیت پرسنل. |
| **Application** | `ShiftYar.Application/DTOs/ProductivityModel/WorkingHoursCalculationResultDto.cs` | مدل خروجی شامل ساعات پایه، کسورات و موظفی خالص نهایی. |
| **Application** | `ShiftYar.Application/DTOs/ProductivityModel/WorkingHoursCalculationBreakdownDto.cs` | ریز جزئیات تفکیک‌شده (کسر سابقه، کسر سختی، کسر شیفت، توضیحات متنی). |
| **Application** | `ShiftYar.Application/Interfaces/ProductivityModel/IWorkingHoursCalculator.cs` | واسط DI سرویس محاسبه. |
| **Application** | `ShiftYar.Application/Features/ProductivityModel/Services/WorkingHoursCalculator.cs` | پیاده‌سازی فرمول‌های دقیق پایه و کسورات بر اساس دستورالعمل وزارت بهداشت. |
| **Application** | `ShiftYar.Application/Common/Utilities/ProductivityRequiredHoursResolver.cs` | اعمال مقدار دستی `MaxProductivityRequiredHours` یا محاسبه خودکار در قیود کاربر. |
| **Application** | `ShiftYar.Application/Common/Utilities/ProductivityWorkedHoursCalculator.cs` | محاسبه ساعت مؤثر کارکرد شیفت‌ها با احتساب ضرایب شب و روز تعطیل و ساعت تحویل کار. |
| **Application** | `ShiftYar.Application/Features/ShiftModel/SimulatedAnnealing/ProjectPersonnelProductivityPriority.cs` | اعمال تقدم غیرطرحی بر طرحی و سقف ساعات مجاز. |
| **Application** | `ShiftYar.Application/Features/ShiftModel/Services/ShiftSchedulingService.cs` | استخراج تقویم بازه، جمعه‌ها، تعطیلات و فراخوانی محاسبه در متد `LoadConstraints`. |

---

## ۷. مثال‌های عددی گام‌به‌گام

### مثال ۱: پرستار بخش ICU با ۱۰ سال سابقه در ماه ۳۰ روزه (گردشی)
- **مشخصات تقویم:** ماه ۳۰ روزه با ۴ روز جمعه و ۲ روز تعطیل رسمی وسط هفته.
  - تعداد کل روزها: ۳۰
  - روزهای کاری غیرتعطیل: $30 - (4 + 2) = 24$ روز.
- **مشخصات پرسنل:** ۱۰ سال سابقه بالینی (بازه ۸ سال و ۱ ماه تا ۱۲ سال)، بخش ویژه ICU یا صعوبت کار بالای ۷۵٪ (یا بالای ۱۰۰۰ امتیاز)، شیفت چرخشی سه نوبته، مشمول بهره‌وری.
- **مراحل محاسبه:**
  1. **ساعت پایه ناخالص ماه:**
     $$24 \times \frac{22}{3} = 24 \times 7.333333 = 176.00 \text{ ساعت}$$
  2. **کسورات هفتگی بر اساس دستورالعمل وزارت بهداشت:**
     - سابقه خدمت (۱۰ سال): **۳٫۰ ساعت** (بازه ۸ سال و ۱ ماه تا ۱۲ سال)
     - صعوبت کار (بخش ویژه / صعوبت بالا): **۲٫۰ ساعت**
     - الگوی نوبت‌کاری (سه نوبته گردشی): **۱٫۰ ساعت** (بدون پیش‌شرط سابقه)
     - مجموع تخفیف هفتگی: $3.0 + 2.0 + 1.0 = \mathbf{6.0}$ ساعت (کمتر از سقف ۸ ساعت)
  3. **کسر ماهانه ناشی از تخفیف هفتگی:**
     $$\frac{30}{7} \times 6.0 = 4.2857 \times 6.0 = 25.71 \text{ ساعت}$$
  4. **ساعت موظفی خالص نهایی ماه:**
     $$176.00 - 25.71 = 150.29 \text{ ساعت} \implies \mathbf{150} \text{ ساعت (گردشده)}$$

---

### مثال ۲: پرسنل اداری / عادی در همان تقویم (غیرمشمول)
- **مشخصات تقویم:** ۲۴ روز کاری غیرتعطیل.
- **مشخصات پرسنل:** `IsIncludedInProductivityPlan = false`.
- **مراحل محاسبه:**
  1. کسورات بهره‌وری = ۰
  2. ساعت کار پایه ناخالص: $24 \times \frac{22}{3} = 176.00$ ساعت
  3. ساعت موظفی خالص نهایی ماه: **۱۷۶٫۰۰ ساعت**.

---

### مثال ۳: پرسنل با تنظیم دستی `MaxProductivityRequiredHours`
- اگر سوپروایزر برای پرسنل مقدار `MaxProductivityRequiredHours = 145` را ثبت کرده باشد:
  - محاسبات تقویمی و کسورات سابقه و سختی نادیده گرفته می‌شوند.
  - ساعت موظفی در شیفت‌بندی دقیقاً **۱۴۵٫۰۰ ساعت** ثبت می‌شود.

---

### مثال ۴: سناریوی واقعی شهریور ۱۴۰۵ (تطابق کامل بر مبنای دستورالعمل اجرایی وزارت بهداشت)
- **مشخصات تقویم:** ۳۱ روز کل، ۴ جمعه، ۱ روز تعطیل رسمی (۸ شهریور).
  - روزهای غیرتعطیل تقویم: $31 - (4 + 1) = 26$ روز.
  - **اعمال سقف قانونی ماه استاندارد:** روزهای موظف روی **۲۴ روز** و ساعت پایه روی **۱۷۶٫۰۰ ساعت** محدود می‌شود.
  - ضریب تخفیف ماهانه برای هر ساعت تخفیف هفتگی: $\frac{31}{7} \approx 4.42857$ ساعت در ماه.
- **جدول تطابق پرسنل بخش ۲ (اورژانس اطفال - شهریور ۱۴۰۵ با فرض صعوبت کار ۱۰۰٪ یا بالای ۱۰۰۰ امتیاز):**

| نام پرسنل | سابقه (سال) | نوع نوبت‌کاری | تخفیف هفتگی | محاسبه موظفی نهایی | ساعت محاسبه در سامانه | وضعیت تطابق |
|:----------|:-----------:|:--------------:|:------------:|:-------------------:|:------------------------:|:-----------:|
| **مهدی رستمی** | ۱ | ۳ نوبته | ۴ ساعت | $176 - 17.71$ | **۱۵۸ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **محمد یوسفی** | ۱ | ۳ نوبته | ۴ ساعت | $176 - 17.71$ | **۱۵۸ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **مهدی دریکوند** | ۱ | ۳ نوبته | ۴ ساعت | $176 - 17.71$ | **۱۵۸ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **زهرا باقری** | ۱ | ۳ نوبته | ۴ ساعت | $176 - 17.71$ | **۱۵۸ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **فائزه سبزواری** | ۲ | ۳ نوبته | ۴ ساعت | $176 - 17.71$ | **۱۵۸ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **کیمیا کاظمی** | ۲ | ۳ نوبته | ۴ ساعت | $176 - 17.71$ | **۱۵۸ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **فاطمه دبستانیان** | ۲ | ۳ نوبته | ۴ ساعت | $176 - 17.71$ | **۱۵۸ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **شکیبا موسیوند** | ۳ | ۳ نوبته | ۴ ساعت | $176 - 17.71$ | **۱۵۸ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **مریم کرمی** | ۴ | ۳ نوبته | ۴ ساعت | $176 - 17.71$ | **۱۵۸ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **مریم امیدی منش** | ۴ | ۳ نوبته | ۴ ساعت | $176 - 17.71$ | **۱۵۸ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **فاطمه رازانی** | ۵ | ۳ نوبته | ۵ ساعت | $176 - 22.14$ | **۱۵۴ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **فاطمه مدهنی** | ۱۰ | ۳ نوبته | ۶ ساعت | $176 - 26.57$ | **۱۴۹ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **حدیث کاظمی** | ۱۰ | ۳ نوبته | ۶ ساعت | $176 - 26.57$ | **۱۴۹ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **عاطفه رحیمی منفرد** | ۱۰ | ۳ نوبته | ۶ ساعت | $176 - 26.57$ | **۱۴۹ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **فاطمه رضایی** | ۱۱ | ۳ نوبته | ۶ ساعت | $176 - 26.57$ | **۱۴۹ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **خدیجه متقی** | ۱۱ | ۳ نوبته | ۶ ساعت | $176 - 26.57$ | **۱۴۹ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **فاطمه سلیمی** | ۱۳ | ۳ نوبته | ۷ ساعت | $176 - 31.00$ | **۱۴۵ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **فرشته ساکی** | ۲۲ | ثابت صبح | ۷ ساعت | $176 - 31.00$ | **۱۴۵ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **صبا حاتمی فیضی** | ۲۴ | ثابت صبح | ۷ ساعت | $176 - 31.00$ | **۱۴۵ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **زهرا درخشانی الوار** | ۲۵ | ۲ نوبته | ۸ ساعت (سقف) | $176 - 35.43$ | **۱۴۱ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |
| **بهاره بهاری پور** | ۱۸ | ۳ نوبته | ۸ ساعت (سقف) | $176 - 35.43$ | **۱۴۱ ساعت** | ✅ کاملاً منطبق بر دستورالعمل |

---

### مثال ۵: ماه ۳۰ روزه با ۲۵ روز کاری تقویمی (محاسبه صرف تقویمی بدون سقف دستی)
- **مشخصات تقویم:** ۳۰ روز کل، ۴ جمعه، ۱ تعطیل رسمی غیرجمعه.
  - روزهای کاری تقویمی: $30 - (4 + 1) = 25$ روز کاری.
  - `CapBaseHoursToStandardMonth = false` (حالت تقویمی).
- **ساعت پایه ناخالص تقویمی:**
  $$\text{BaseMonthlyHours} = 25 \times \frac{22}{3} \approx 183.33 \text{ ساعت}$$
- **ضریب کسر ماهانه:** $\frac{30}{7} \approx 4.2857$ ساعت به ازای هر ۱ ساعت تخفیف هفتگی.
- **جدول موظفی پرسنل:**
  - بدون تخفیف (۰ ساعت): **۱۸۳٫۳۳ ساعت** (گردشده: ۱۸۳)
  - با ۱ ساعت تخفیف: $183.33 - 4.29 = \mathbf{179.05}$ ساعت (گردشده: ۱۷۹)
  - با ۲ ساعت تخفیف: $183.33 - 8.57 = \mathbf{174.76}$ ساعت (گردشده: ۱۷۵)
  - با ۳ ساعت تخفیف: $183.33 - 12.86 = \mathbf{170.48}$ ساعت (گردشده: ۱۷۰)
  - با ۴ ساعت تخفیف: $183.33 - 17.14 = \mathbf{166.19}$ ساعت (گردشده: ۱۶۶)

---

### مثال ۶: ماه ۲۹ روزه (اسفند ماه)
- **مشخصات تقویم:** ۲۹ روز کل، ۴ جمعه، ۲ تعطیل رسمی غیرجمعه.
  - روزهای کاری تقویمی: $29 - (4 + 2) = 23$ روز کاری.
- **ساعت پایه ناخالص تقویمی:**
  $$\text{BaseMonthlyHours} = 23 \times \frac{22}{3} \approx 168.67 \text{ ساعت}$$
- **ضریب کسر ماهانه تقویمی:**
  $$\text{WeeklyToMonthlyRatio} = \frac{29}{7} \approx 4.1429$$
- **جدول موظفی پرسنل:**
  - بدون تخفیف: **۱۶۸٫۶۷ ساعت** (گردشده: ۱۶۹)
  - با ۱ ساعت تخفیف: $168.67 - 4.14 = \mathbf{164.53}$ ساعت (گردشده: ۱۶۵)
  - با ۲ ساعت تخفیف: $168.67 - 8.29 = \mathbf{160.38}$ ساعت (گردشده: ۱۶۰)
  - با ۳ ساعت تخفیف: $168.67 - 12.43 = \mathbf{156.24}$ ساعت (گردشده: ۱۵۶)
  - با ۴ ساعت تخفیف: $168.67 - 16.57 = \mathbf{152.10}$ ساعت (گردشده: ۱۵۲)


