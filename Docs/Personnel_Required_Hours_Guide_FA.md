...
### سناریو ۵: محاسبه واقعی پرسنل مشمول طرح بهره‌وری در ماه ۳۰ روزه با ۲۶ روز کاری (نمونه مهر ۱۴۰۵ - بخش هوشبری)
- **مشخصات تقویم:**
  * طول ماه: ۳۰ روز (مهر ماه ۱۴۰۵)
  * روزهای جمعه: ۴ جمعه
  * تعطیلات رسمی وسط هفته: ۰ روز
  * تعداد روزهای کاری موظف تقویمی: $30 - 4 = \mathbf{26}$ روز کاری
  * ساعت موظفی خام (Gross):
    $$\text{Gross} = 26 \times \left(\frac{44}{6}\right) = 26 \times 7.333333 = \mathbf{190.67} \text{ ساعت}$$
- **اطلاعات پرسنل (جواد انصاری):**
  * سابقه خدمت: حدود ۲ سال (بازه ۰ تا ۴ سال) $\rightarrow$ **۱٫۰ ساعت** کسر سنوات هفتگی
  * شمول در طرح بهره‌وری بالینی (`IncludedProductivityPlan = true`): فعال بودن شمول طرح بهره‌وری (کلید فعال‌سازی قانون).
  * تخفیف پیش‌فرض بخش عمومی بالینی: `GeneralSectionHardshipReduction = 0.0m` (تعیین سختی کار منوط به ثبت درصد یا امتیاز شغلی است).
  * الگوی شیفت: شیفت ثابت صبح (`FixedMorning`) $\rightarrow$ **۰٫۰ ساعت** کسر نوبت‌کاری

- **حالت الف: پیش از ثبت درصد سختی کار (`HardshipPercent = null`):**
  * کسر سختی کار: **۰٫۰ ساعت**
  * مجموع تخفیف هفتگی بهره‌وری: $1.0 + 0.0 + 0.0 = \mathbf{1.0}$ ساعت در هفته
  * کسر ماهانه (۴ هفته استاندارد): $1.0 \times 4 = \mathbf{4.0}$ ساعت
  * ساعت موظفی خالص ماهانه:
    $$\text{Net} = 190.67 - 4.0 = 186.67 \implies \mathbf{187} \text{ ساعت (گردشده)}$$

- **حالت ب: پس از ثبت درصد سختی کار هوشبری (مثلاً ۳۰٪ - بازه ۲۶ تا ۵۰ درصد):**
  * کسر سختی کار: **۱٫۰ ساعت** در هفته
  * مجموع تخفیف هفتگی بهره‌وری: $1.0 + 1.0 + 0.0 = \mathbf{2.0}$ ساعت در هفته
  * کسر ماهانه (۴ هفته استاندارد): $2.0 \times 4 = \mathbf{8.0}$ ساعت
  * ساعت موظفی خالص ماهانه:
    $$\text{Net} = 190.67 - 8.0 = 182.67 \implies \mathbf{183} \text{ ساعت (گردشده)}$$
  * نتیجه در خروجی الگوریتم زمان‌بندی: `UserConstraint.ProductivityRequiredHours = 183` و `MonthlyCalendarSnapshot` با جزئیات کامل ذخیره و اعمال می‌شود.
