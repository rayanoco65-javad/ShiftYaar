...
| ساعات موظفی بهره‌وری | `StaffEmploymentInfoDto` + سرویس Productivity | برای هر ماه باید عدد هفته‌ها، ساعات شب و فیلدهای سختی کار پر شود. فیلد **`MaxProductivityRequiredHours`** روی کاربر امکان override دستی موظفی را می‌دهد. |
| نوع پرسنل (`IsProjectPersonnel`) | فرم کاربر | غیرطرحی اولویت پر کردن موظفی؛ طرحی فقط تا موظفی (بدون اضافه‌کار). |
...
### ۲.۱ قوانین جدید درخواست شیفت و موظفی

- **OFF تأییدشده** برای **صبح** یا **کل‌روز** → **شب روز قبل** به کاربر داده نمی‌شود (`ApprovedOffNightBeforeRules`).
- **تأیید حضور** فقط تا **ظرفیت شیفت/تخصص** همان روز (`ApprovedOnShiftCapacityValidator`).
- **تأیید عدم‌حضور (مرخصی روزانه)** فقط تا **حداکثر سقف مجاز روزانه تخصص** (`ApprovedLeaveCapacityValidator`). سقف مجاز برابر است با تفاضل کل پرسنل فعال تخصص و حداقل پرسنل مورد نیاز (مجموع نیاز شیفت‌های امروز + استراحت اجباری شیفت شب روز قبل). در صورت تکمیل سقف، سیستم با خطای ۴۰۰ و پیام فارسی حاوی مشخصات کامل و اسامی افراد دارای مرخصی مانع از تأیید می‌شود.
- **قید سخت سقف شیفت روزانه بر اساس تقویم شمسی:** در صورت فعال بودن `EnforceMaxShiftsPerDay` با مقدار `1`، هر کاربر در طول یک شبانه‌روز تقویم هجری شمسی ($۰۰:۰۰$ تا $۲۳:۵۹$) حداکثر مجاز به داشتن ۱ شیفت است و هیچ استثنایی برای ایجاد شیفت‌های لانگ (صبح+عصر) وجود ندارد.
- **اولویت مطلق درخواست‌های تأییدشده (Approved Shift Requests):**
  - **پرسنل شیفت فیکس در روزهای تعطیل:** پرسنل فیکس به‌طور پیش‌فرض در روزهای تعطیل رسمی آف هستند. اگر برای یک روز تعطیل درخواست حضور تأییدشده وجود داشته باشد، آن درخواست در برنامه اعمال می‌شود و کاربر در آن روز آف نخواهد بود. همچنین در صورت درخواست شیفت مشخص (مثلاً عصر برای فیکس صبح)، همان شیفت تخصیص می‌یابد.
  - **پرسنل شیفت گردشی و مجوزها:** اگر کاربر گردشی طبق تنظیمات کاربری تنها مجاز به شیفت‌های خاصی باشد (مثلاً فقط عصر و شب، بدون مجوز صبح)، در صورت وجود درخواست تأییدشده برای شیفت صبح، درخواست تأییدشده بر مجوز شیفت اولویت قطعی داشته و در برنامه اعمال می‌گردد.
  - **استثنای قانون تناوب هفتگی شیفت‌های روزانه:** در صورتی که برای کاربر قانون تناوب هفتگی فعال باشد (مثلاً در هفته دوم فقط مجاز به شیفت عصر باشد)، در صورت وجود یک درخواست شیفت تأییدشده برای شیفت متضاد (مثلاً صبح)، درخواست تأییدشده بر قانون تناوب اولویت قطعی داشته و شیفت درخواستی در برنامه اعمال می‌شود.
  - **تطبیق خودکار ظرفیت روزهای تعطیل:** ظرفیت شیفت در روزهای تعطیل متناسب با درخواست‌های تأییدشده گسترش می‌یابد تا از خطای مازاد ظرفیت جلوگیری شود.
- **قانون تخصیص شیفت‌های متناوب هفتگی (صبح و عصر) با تعیین شیفت آغازین:** در جدول سهمیه روزانه پرسنل (`UserMonthlyDayShiftQuota`)، با فعال‌سازی `IsWeeklyAlternatingActive` و انتخاب شیفت هفته اول (`FirstWeekShiftLabel` برابر Morning=0 یا Evening=1)، سیستم شیفت‌های روزانه کاربر را بر اساس تقویم استاندارد شمسی (شنبه تا جمعه با پوشش هفته‌های ناقص) به صورت متناوب (یک هفته صبح / یک هفته عصر) تخصیص می‌دهد. **شیفت‌های شب، آف و مرخصی‌ها کاملاً از این قاعده مستثنی هستند** و بدون تداخل طبق روال عادی تخصیص می‌یابند.
- **توقف عملیات شیفت‌بندی در نقض قوانین سخت:** نقض قوانینی مانند سقف روزهای کاری متوالی (`MaxConsecutiveWorkdays`) یا سقف شیفت روزانه صرفاً در حد هشدار نبوده بلکه به عنوان قید سخت (Hard Constraint) منجر به توقف کامل شیفت‌بندی با خطای صریح می‌شود.
- **توزیع هوشمند اضافه‌کار بر اساس سابقه خدمت (سنوات) و تمایل دپارتمان (`OvertimePreferenceType`):** با تنظیم نوع ترجیح روی ۰ یا ۱، سیستم به صورت خودکار توزیع سنواتی اضافه‌کار را فعال می‌کند:
  - **دپارتمان‌های علاقه‌مند به اضافه‌کار (`OvertimeFriendly = 0`):** پرسنل با سابقه خدمت بیشتر، سهم بیشتری از شیفت‌های مازاد بر موظفی (اضافه‌کار) دریافت می‌کنند ($Overtime \propto Seniority$).
  - **دپارتمان‌های گریزان از اضافه‌کار (`OvertimeAvoiding = 1`):** پرسنل با سابقه خدمت کمتر، شیفت‌های مازاد را پوشش می‌دهند تا پرسنل باسابقه در سقف موظفی قانونی (اضافه کار نزدیک به صفر) باقی بمانند ($Overtime \propto \frac{1}{Seniority}$). سیستم حتی اختلاف ساعات موظفی پرسنل در قانون بهره‌وری را مهار کرده و مانع از اضافه‌کار ناخواسته با‌سابقه‌ها می‌شود.
  - **دپارتمان‌های خنثی / توزیع برابر (`Neutral = 2`):** اضافه‌کار به صورت مساوی و یکنواخت بین پرسنل متقاضی توزیع می‌شود.
  - **شیب اختصاصی تقسیم اضافه‌کار (`OvertimeSeniorityDistributionSlope`):** سوپروایزر می‌تواند شدت و شیب صعودی یا نزولی بودن تقسیم اضافه‌کار را به طور مستقل از شیب عمومی دپارتمان تنظیم کند (بازه مجاز: $0.1$ تا $10.0$ با پیش‌فرض $1.0$). مقادیر بالاتر (مانند $1.5$ تا $2.5$) تفاوت بین سهم اضافه‌کار پرسنل باسابقه و کم‌سابقه را تشدید کرده و تفکیک حداکثری ایجاد می‌کنند، در حالی که مقادیر پایین‌تر (مانند $0.5$) توزیع ملایم‌تری را به همراه دارند.
  - **هماهنگی با رضایت فردی (`OvertimeConsent`):** پرسنلی که گزینه رضایت به اضافه‌کار را غیرفعال کرده‌اند (`OvertimeConsent = false`)، تحت هیچ شرایطی اضافه‌کار دریافت نکرده و شیفت‌های مازاد به متقاضیان منتقل می‌گردد.
- **اولویت موظفی:** پرسنل غیرطرحی (`IsProjectPersonnel = false/null`) قبل از طرحی؛ پرسنل طرحی حداکثر تا ساعت موظفی.
- **ساعت موظفی دستی:** `MaxProductivityRequiredHours` — اگر مثبت باشد، محاسبه خودکار نادیده گرفته می‌شود.
- **مجوز نوع شیفت:** فیلد `AllowedShiftPermissions` (flags) — صبح/عصر/شب/صبح‌عصر/صبح‌شب per user.

جزئیات UI/API: `Docs/FrontendGuide_ProductivityAndScheduling_FA.md`.

---

## ۳. فرایند کلی کار سیستم

1. **جمع‌آوری داده‌ها:** API یا داشبورد مدیریتی اطلاعات کاربران، تخصص‌ها، سهمیه شب ماهانه، درخواست‌های مرخصی/حضور و اولویت‌ها را دریافت می‌کند.
...
1. **ثبت داده پایه** (کاربر، دپارتمان، تخصص، الگوهای شیفت، تعطیلات/`ShiftDates`).
2. **وارد کردن تنظیمات دپارتمان:** API `DepartmentSchedulingSettings` (سقف‌ها، بهره‌وری، و در صورت نیاز `allowCurrentMonthScheduling` / `allowMonthlyRescheduleWithAutoDelete`).
3. **سهمیه شب ماهانه:** `UserMonthlyNightQuota` برای ماه شمسی موردنظر (مجموع ≤ تعداد شب‌های ماه؛ پیش‌نیاز درخواست شب).
4. **سهمیه شیفت روزانه و تناوب هفتگی:** `UserMonthlyDayShiftQuota` برای ماه شمسی (سهمیه‌های قطعی صبح و عصر، تعطیلات، و فعال‌سازی قانون تناوب هفتگی صبح ↔ عصر با تعیین شیفت آغازین هفته اول؛ با در نظر گرفتن استثنای کامل شیفت شب و اولویت مطلق درخواست‌های تأییدشده).
5. **محاسبه ساعات موظفی:** برای هر ماه، سرویس Productivity را صدا بزنید:
   ```csharp
   ...
   ```
6. **درخواست‌های شیفت:** ثبت و تأیید (حضور فقط شیفت‌مشخص؛ شب داخل سهمیه؛ حضور کل‌روز ممنوع؛ درخواست‌های تأییدشده بر تناوب هفتگی و مجوزها ارجحیت دارند).
7. **ارسال درخواست زمان‌بندی:** `optimize-and-save` با دپارتمان، بازه یک ماه شمسی و الگوریتم؛ سهمیه شب اجباری است.
8. **بازبینی:** در صورت نیاز حذف ماهانه (`DeleteMonthlySchedule`) و اجرای مجدد.
