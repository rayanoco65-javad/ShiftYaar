...
- **توقف عملیات شیفت‌بندی در نقض قوانین سخت:** نقض قوانینی مانند سقف روزهای کاری متوالی (`MaxConsecutiveWorkdays`) صرفاً در حد هشدار نبوده بلکه به عنوان قید سخت (Hard Constraint) منجر به توقف کامل شیفت‌بندی با خطای صریح می‌شود.
