...
- **ارتباط با درخواست شیفت شب** (پیام خطا را عیناً به سوپروایزر نشان دهید):
  ...
  - **ثبت و تأیید** درخواست شب بدون سهمیه ماهانه، یا بیش از سهمیه کل / سهمیه تعطیل، رد می‌شود.
  - اگر سهمیه تعطیل > 0 باشد، تعداد درخواست شب **غیرتعطیل** نباید از `ExactNight − ExactHoliday` بیشتر شود (مثال: شب=۵ و تعطیل=۱ ⇒ حداکثر ۴ شب غیرتعطیل).
  - هنگام **تعیین/ویرایش/حذف سهمیه**: نمی‌توان سهمیه را کمتر از تعداد درخواست‌های تأییدشده گذاشت یا در حضور درخواست تأییدشده حذف کرد؛ ترکیب سهمیه نباید با شب‌های غیرتعطیل تأییدشده تناقض داشته باشد.
