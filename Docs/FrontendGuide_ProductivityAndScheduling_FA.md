...
| `allowNightShiftAfterNightShift` | اجازه شیفت شب در روز بعد از شب | **`bool` (غیر nullable)** — پیش‌فرض `false`: شب پشت شب **ممنوع**. `true`: **شب** روز بعد از شب مجاز است و فاصلهٔ اجباری ۲ روزه بین شب‌ها برای سهمیه شب برداشته می‌شود (سقف زنجیره: `maxConsecutiveNightShifts`). |
