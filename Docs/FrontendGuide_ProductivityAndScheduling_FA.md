...
برای مدیریت هوشمند شیفت‌های مازاد بر موظفی (اضافه‌کار)، سه فیلد جدید به تنظیمات زمان‌بندی دپارتمان (`DepartmentSchedulingSettings`) افزوده شده است:
...
1. **کنترل تمایل دپارتمان:** در فرم تنظیمات دپارتمان، فیلد `overtimePreferenceType` به صورت یک Dropdown یا Radio Group سه‌حالتی پیاده‌سازی شود:
...
2. **ارتباط با رضایت فردی (`OvertimeConsent`):**
