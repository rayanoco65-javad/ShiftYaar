...
- `ExactHolidayWeekendNightShiftCount`: از میان شیفت‌های شب، چند مورد باید در **تعطیل/آخرهفته** باشد (روزهایی که در تقویم `IsHoliday=true` هستند، از جمله جمعه). باید ≤ `ExactNightShiftCount` باشد.
