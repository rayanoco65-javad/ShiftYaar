...
text = re.sub(r'private ShiftSolution GenerateFeasibleInitialSolution\(\)\s*\{[^{}]*const int maxAttempts[^}]*return best \?\? GenerateInitialSolution\(\);\s*\}', new_code, text, flags=re.DOTALL)

with open(path, 'w', encoding='utf-8') as f:
    f.write(text)
print('Injected telemetry into initial solution generator')
