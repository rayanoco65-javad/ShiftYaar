...
public static class ExactNightQuotaGuard
{
    ...
    private static void Log(string msg) => LogAction?.Invoke(msg);
    ...
    public static void Enforce(ShiftSolution solution, ShiftConstraints constraints)
    {
        ...
        for (var pass = 0; pass < 4; pass++)
        {
            ...
            foreach (var user in deficitUsers)
            {
                ...
                {
                    if (!TrySatisfyDeficitViaOpenSlotBridge(solution, constraints, user, nightShift))
                    {
                        TryClaimViaSurplusRotation(solution, constraints, user, nightShift);
                    }
                }
            }
        }
        ...
        bool allowCriticalDonor = false)
    {
        ...
        {
            if (TryClaimNightDirectOrFromDonor(solution, constraints, user, nightShift, date, allowCriticalDonor))
            {
                return true;
            }
        }
    ...
    private static List<SaShiftAssignment> Snapshot3DayWindow(ShiftSolution solution, int userId, DateTime centerDate) =>
        solution.GetUserAllAssignments(userId)
            .Where(a => Math.Abs((a.Date.Date - centerDate.Date).Days) <= 1 && !a.IsOnCall)
            .ToList();

    private static void Rollback3DayWindow(ShiftSolution solution, int userId, DateTime centerDate, List<SaShiftAssignment> snapshot)
    {
        foreach (var cur in solution.GetUserAllAssignments(userId).Where(a => Math.Abs((a.Date.Date - centerDate.Date).Days) <= 1 && !a.IsOnCall).ToList())
        {
            solution.UnlockSkeletonAssignment(userId, cur.ShiftId, cur.Date);
            solution.RemoveAssignment(userId, cur.ShiftId, cur.Date, force: true);
        }
        foreach (var orig in snapshot)
        {
            solution.AddAssignment(userId, orig.ShiftId, orig.Date, orig.ShiftLabel, orig.IsOnCall, isSkeleton: orig.IsSkeleton);
        }
    }

    private static bool TryClaimNightDirectOrFromDonor(
        ShiftSolution solution,
        ShiftConstraints constraints,
        UserConstraint user,
        ShiftRequirement nightShift,
        ...
        bool allowCriticalDonor)
    {
        ...
        {
            var userSnapshot = Snapshot3DayWindow(solution, user.UserId, date);
            ...
            var mixOk = IsSlotManagerMixSatisfied(solution, constraints, nightShift, date);
            var countOk = CountNights(solution, user.UserId) > receiverBefore;
            var noOverCap = !ShiftCoverageGuard.HasAnyOverCapacity(solution, constraints);

            if (restOk && runOk && mixOk && countOk && noOverCap)
            {
                return true;
            }

            // بازگرداندن دقیق پنجره ۳ روزه (Delta undo)
            solution.RemoveAssignment(user.UserId, nightShift.ShiftId, date, force: true);
            Rollback3DayWindow(solution, user.UserId, date, userSnapshot);
            return false;
        }

        // ۲) اهداکننده دارای مازاد
        var donorAssignment = FindMandatoryDonor(solution, constraints, nightShift, date, user.UserId, visitedUsers: null, allowCriticalDonor);
        if (donorAssignment != null)
        {
            ...
            if (donor != null)
            {
                ...
                var donorWasSkeleton = donorAssignment.IsSkeleton;
                ...
                if (donorOk && restOk && runOk && mixOk && countOk && noOverCap)
                {
                    return true;
                }
                ...
                solution.RemoveAssignment(user.UserId, nightShift.ShiftId, date, force: true);
                Rollback3DayWindow(solution, user.UserId, date, userSnapshot);
                solution.AddAssignment(donor.UserId, nightShift.ShiftId, date, ShiftLabel.Night, false, isSkeleton: donorWasSkeleton);
            }
        }
        ...
        foreach (var vacantDate in dates)
        {
            ...
            for (var s = 0; s < openSlots && totalDeficit > 0; s++)
            {
                ...
                foreach (var user in candidateUsers)
                {
                    ...
                    if (!HasSpecialtyCapacity(solution, constraints, nightShift, vacantDate, user.SpecialtyId))
                    {
                        continue;
                    }
                    ...
                    if (!IsPersonallyFeasibleNightDate(solution, constraints, user, nightShift, vacantDate, holidayOnly: false)
                        || !CanAcceptNightAfterClearing(solution, constraints, user, nightShift, vacantDate))
                    {
                        continue;
                    }

                    var userSnapshot = Snapshot3DayWindow(solution, user.UserId, vacantDate);
                    var mixBefore = IsSlotManagerMixSatisfied(solution, constraints, nightShift, vacantDate);
                    ...
                    var runOk = !constraints.HardRules.EnforceMaxConsecutiveShifts
                        || MaxConsecutiveWorkdayRules.GetMaxConsecutiveWorkRun(solution, user.UserId) <= user.MaxConsecutiveShifts;
                    var mixOk = IsSlotManagerMixSatisfied(solution, constraints, nightShift, vacantDate) || !mixBefore;
                    var noOverCap = !ShiftCoverageGuard.HasAnyOverCapacity(solution, constraints);

                    if (restOk && runOk && mixOk && noOverCap)
                    {
                        slotFilled = true;
                        anyAdded = true;
                        totalDeficit--;
                    ...
                    // بازگرداندن تغییرات (Delta undo)
                    solution.RemoveAssignment(user.UserId, nightShift.ShiftId, vacantDate, force: true);
                    Rollback3DayWindow(solution, user.UserId, vacantDate, userSnapshot);
                }
    ...
    /// <summary>
    /// شب مازاد یک کاربر دیگر را — با جابجایی ۱ یا ۲ مرحله‌ای مستقیم — به گیرنده منتقل می‌کند.
    /// بدون ایجاد کلون‌های سنگین در حافظه (از Delta Undo استفاده می‌شود).
    /// </summary>
        ...
        ShiftRequirement nightShift)
    {
        ...
        foreach (var (donor, night) in EnumerateSurplusNights(solution, constraints, excludeUserId: receiver.UserId))
        {
            ...
            // ۱) تلاش برای انتقال مستقیم شب مازاد به گیرنده (1-hop)
            ...
            {
                var donorWasSkeleton = night.IsSkeleton;
                var recSnapshot = Snapshot3DayWindow(solution, receiver.UserId, date);
                ...
                // Delta undo
                solution.RemoveAssignment(receiver.UserId, nightShift.ShiftId, date, force: true);
                Rollback3DayWindow(solution, receiver.UserId, date, recSnapshot);
                solution.AddAssignment(donor.UserId, nightShift.ShiftId, date, ShiftLabel.Night, false, isSkeleton: donorWasSkeleton);
            }

            // ۲) اگر گیرنده مستقیماً نتوانست شب مازاد را بگیرد، از چرخش ۲ مرحله‌ای استفاده کن (2-hop):
            // donor (date) -> intermediate (date), intermediate (date2) -> receiver (date2)
            foreach (var intermediate in constraints.UserConstraints.Where(u => u.IsActive && u.UserId != donor.UserId && u.UserId != receiver.UserId))
            {
                ...
                if (!HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, date, intermediate.SpecialtyId, donor.UserId))
                {
                    continue;
                }
                ...
                foreach (var date2 in intermediateNights)
                {
                    ...
                    if (!HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, date2, receiver.SpecialtyId, intermediate.UserId))
                    {
                        continue;
                    }
                    ...
                    var donorWasSkeleton = night.IsSkeleton;
                    var intWasSkeleton = intAsg != null && intAsg.IsSkeleton;

                    var intSnapshotDate = Snapshot3DayWindow(solution, intermediate.UserId, date);
                    var intSnapshotDate2 = Snapshot3DayWindow(solution, intermediate.UserId, date2);
                    var recSnapshot = Snapshot3DayWindow(solution, receiver.UserId, date2);

                    solution.UnlockSkeletonAssignment(donor.UserId, night.ShiftId, night.Date);
                    solution.RemoveAssignment(donor.UserId, night.ShiftId, night.Date, force: true);

                    ClearConflictingForNight(solution, constraints, intermediate, date);
                    ...
                    ClearConflictingForNight(solution, constraints, receiver, date2);
                    ...
                    // Delta undo
                    solution.RemoveAssignment(receiver.UserId, nightShift.ShiftId, date2, force: true);
                    solution.RemoveAssignment(intermediate.UserId, nightShift.ShiftId, date, force: true);
                    Rollback3DayWindow(solution, receiver.UserId, date2, recSnapshot);
                    Rollback3DayWindow(solution, intermediate.UserId, date, intSnapshotDate);
                    Rollback3DayWindow(solution, intermediate.UserId, date2, intSnapshotDate2);
                    solution.AddAssignment(donor.UserId, nightShift.ShiftId, date, ShiftLabel.Night, false, isSkeleton: donorWasSkeleton);
                }
            }
        }

        // ۳) اگر چرخش ۱ و ۲ مرحله‌ای مستقیم جواب نداد، از چرخش ۳ مرحله‌ای مستقیم (3-hop) استفاده کن
        if (TryClaim3HopRotation(solution, constraints, receiver, nightShift))
        {
            return true;
        }

        // ۴) زنجیره چرخشی چندمرحله‌ای (Augmenting Path) به عنوان آخرین راهکار
    ...
    /// <summary>
    /// چرخش ۳ مرحله‌ای مستقیم:
    /// donor (date1) -> userX (date1), userX (date2) -> userY (date2), userY (date3) -> receiver (date3)
    /// فوق‌العاده سریع با هرس زودهنگام ترکیب مسئولین و محدودیت‌های سخت.
    /// </summary>
    private static bool TryClaim3HopRotation(
        ShiftSolution solution,
        ShiftConstraints constraints,
        UserConstraint receiver,
        ShiftRequirement nightShift)
    {
        ...
        foreach (var (donor, night) in EnumerateSurplusNights(solution, constraints, excludeUserId: receiver.UserId))
        {
            ...
            foreach (var userX in constraints.UserConstraints.Where(u => u.IsActive && u.UserId != donor.UserId && u.UserId != receiver.UserId))
            {
                var xMinGap = ResolveNightSpacingGap(constraints, userX);
                if (ViolatesNightSpacing(solution, constraints, userX, date1, xMinGap))
                {
                    continue;
                }

                if (!HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, date1, userX.SpecialtyId, donor.UserId))
                {
                    continue;
                }

                if (!IsPersonallyFeasibleNightDate(solution, constraints, userX, nightShift, date1, holidayOnly: false)
                    || !CanAcceptNightAfterClearing(solution, constraints, userX, nightShift, date1))
                {
                    continue;
                }

                var (reqTotal1, minL1_1) = ShiftManagerRules.GetRequirement(nightShift);
                var asgs1 = solution.GetShiftAssignments(nightShift.ShiftId, date1).Where(a => !a.IsOnCall).ToList();
                var mgrs1 = asgs1.Count(a => a.UserId != donor.UserId && ShiftManagerRules.IsManager(constraints.UserConstraints.FirstOrDefault(u => u.UserId == a.UserId)))
                            + (ShiftManagerRules.IsManager(userX) ? 1 : 0);
                var l1_1 = asgs1.Count(a => a.UserId != donor.UserId && ShiftManagerRules.IsLevel1(constraints.UserConstraints.FirstOrDefault(u => u.UserId == a.UserId)))
                           + (ShiftManagerRules.IsLevel1(userX) ? 1 : 0);
                if (mgrs1 < reqTotal1 || l1_1 < minL1_1)
                {
                    continue;
                }

                var xNights = GetNights(solution, userX.UserId).Select(a => a.Date.Date).ToList();
                foreach (var date2 in xNights)
                {
                    if (date2 == date1) continue;

                    var hasXAsg = solution.TryGetAssignment(userX.UserId, nightShift.ShiftId, date2, out var xAsg);
                    if (hasXAsg && IsProtected(constraints, userX.UserId, xAsg))
                    {
                        continue;
                    }

                    var xNightsWithoutDate2 = xNights.Where(d => d != date2).ToList();
                    if (xNightsWithoutDate2.Any(d => Math.Abs((d - date1).Days) <= xMinGap))
                    {
                        continue;
                    }

                    foreach (var userY in constraints.UserConstraints.Where(u => u.IsActive && u.UserId != donor.UserId && u.UserId != receiver.UserId && u.UserId != userX.UserId))
                    {
                        var yMinGap = ResolveNightSpacingGap(constraints, userY);
                        if (ViolatesNightSpacing(solution, constraints, userY, date2, yMinGap))
                        {
                            continue;
                        }

                        if (!HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, date2, userY.SpecialtyId, userX.UserId))
                        {
                            continue;
                        }

                        if (!IsPersonallyFeasibleNightDate(solution, constraints, userY, nightShift, date2, holidayOnly: false)
                            || !CanAcceptNightAfterClearing(solution, constraints, userY, nightShift, date2))
                        {
                            continue;
                        }

                        var asgs2 = solution.GetShiftAssignments(nightShift.ShiftId, date2).Where(a => !a.IsOnCall).ToList();
                        var mgrs2 = asgs2.Count(a => a.UserId != userX.UserId && ShiftManagerRules.IsManager(constraints.UserConstraints.FirstOrDefault(u => u.UserId == a.UserId)))
                                    + (ShiftManagerRules.IsManager(userY) ? 1 : 0);
                        var l1_2 = asgs2.Count(a => a.UserId != userX.UserId && ShiftManagerRules.IsLevel1(constraints.UserConstraints.FirstOrDefault(u => u.UserId == a.UserId)))
                                   + (ShiftManagerRules.IsLevel1(userY) ? 1 : 0);
                        if (mgrs2 < reqTotal1 || l1_2 < minL1_1)
                        {
                            continue;
                        }
                        ...
                        foreach (var date3 in yNights)
                        {
                            ...
                            if (hasYAsg && IsProtected(constraints, userY.UserId, yAsg))
                            {
                                continue;
                            }

                            var yNightsWithoutDate3 = yNights.Where(d => d != date3).ToList();
                            if (yNightsWithoutDate3.Any(d => Math.Abs((d - date2).Days) <= yMinGap))
                            {
                                continue;
                            }

                            if (ViolatesNightSpacing(solution, constraints, receiver, date3, receiverMinGap))
                            {
                                continue;
                            }

                            if (!HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, date3, receiver.SpecialtyId, userY.UserId))
                            {
                                continue;
                            }

                            if (!IsPersonallyFeasibleNightDate(solution, constraints, receiver, nightShift, date3, holidayOnly: false)
                                || !CanAcceptNightAfterClearing(solution, constraints, receiver, nightShift, date3))
                            {
                                continue;
                            }

                            var asgs3 = solution.GetShiftAssignments(nightShift.ShiftId, date3).Where(a => !a.IsOnCall).ToList();
                            var mgrs3 = asgs3.Count(a => a.UserId != userY.UserId && ShiftManagerRules.IsManager(constraints.UserConstraints.FirstOrDefault(u => u.UserId == a.UserId)))
                                        + (ShiftManagerRules.IsManager(receiver) ? 1 : 0);
                            var l1_3 = asgs3.Count(a => a.UserId != userY.UserId && ShiftManagerRules.IsLevel1(constraints.UserConstraints.FirstOrDefault(u => u.UserId == a.UserId)))
                                       + (ShiftManagerRules.IsLevel1(receiver) ? 1 : 0);
                            if (mgrs3 < reqTotal1 || l1_3 < minL1_1)
                            {
                                continue;
                            }

                            // Pre-checks passed! Execute 3-way delta swap
                            var donorWasSkeleton = night.IsSkeleton;
                            var xWasSkeleton = xAsg != null && xAsg.IsSkeleton;
                            var yWasSkeleton = yAsg != null && yAsg.IsSkeleton;

                            var xSnap1 = Snapshot3DayWindow(solution, userX.UserId, date1);
                            var xSnap2 = Snapshot3DayWindow(solution, userX.UserId, date2);
                            var ySnap2 = Snapshot3DayWindow(solution, userY.UserId, date2);
                            var ySnap3 = Snapshot3DayWindow(solution, userY.UserId, date3);
                            var recSnap = Snapshot3DayWindow(solution, receiver.UserId, date3);

                            // 1) Donor drops date1, userX takes date1
                            solution.UnlockSkeletonAssignment(donor.UserId, night.ShiftId, date1);
                            solution.RemoveAssignment(donor.UserId, night.ShiftId, date1, force: true);
                            ClearConflictingForNight(solution, constraints, userX, date1);
                            AddNightSafely(solution, constraints, userX, nightShift, date1);

                            // 2) userX drops date2, userY takes date2
                            solution.UnlockSkeletonAssignment(userX.UserId, night.ShiftId, date2);
                            solution.RemoveAssignment(userX.UserId, night.ShiftId, date2, force: true);
                            ClearConflictingForNight(solution, constraints, userY, date2);
                            AddNightSafely(solution, constraints, userY, nightShift, date2);

                            // 3) userY drops date3, receiver takes date3
                            solution.UnlockSkeletonAssignment(userY.UserId, night.ShiftId, date3);
                            solution.RemoveAssignment(userY.UserId, night.ShiftId, date3, force: true);
                            ClearConflictingForNight(solution, constraints, receiver, date3);
                            AddNightSafely(solution, constraints, receiver, nightShift, date3);

                            var countOk = CountNights(solution, receiver.UserId) > receiverBefore
                                && CountNights(solution, userX.UserId) >= (userX.ExactNightShiftCount ?? 0)
                                && CountNights(solution, userY.UserId) >= (userY.ExactNightShiftCount ?? 0)
                                && CountNights(solution, donor.UserId) >= (donor.ExactNightShiftCount ?? 0);

                            var mix1 = IsSlotManagerMixSatisfied(solution, constraints, nightShift, date1);
                            var mix2 = IsSlotManagerMixSatisfied(solution, constraints, nightShift, date2);
                            var mix3 = IsSlotManagerMixSatisfied(solution, constraints, nightShift, date3);

                            var restX = !AdjacentShiftRestRules.HasForbiddenAdjacentPair(solution.GetUserAllAssignments(userX.UserId), constraints.HardRules);
                            var restY = !AdjacentShiftRestRules.HasForbiddenAdjacentPair(solution.GetUserAllAssignments(userY.UserId), constraints.HardRules);
                            var restRec = !AdjacentShiftRestRules.HasForbiddenAdjacentPair(solution.GetUserAllAssignments(receiver.UserId), constraints.HardRules);

                            var runX = !constraints.HardRules.EnforceMaxConsecutiveShifts || MaxConsecutiveWorkdayRules.GetMaxConsecutiveWorkRun(solution, userX.UserId) <= userX.MaxConsecutiveShifts;
                            var runY = !constraints.HardRules.EnforceMaxConsecutiveShifts || MaxConsecutiveWorkdayRules.GetMaxConsecutiveWorkRun(solution, userY.UserId) <= userY.MaxConsecutiveShifts;
                            var runRec = !constraints.HardRules.EnforceMaxConsecutiveShifts || MaxConsecutiveWorkdayRules.GetMaxConsecutiveWorkRun(solution, receiver.UserId) <= receiver.MaxConsecutiveShifts;

                            if (countOk && mix1 && mix2 && mix3 && restX && restY && restRec && runX && runY && runRec
                                && !ShiftCoverageGuard.HasAnyOverCapacity(solution, constraints))
                            {
                                return true;
                            }
                            ...
                            // Delta undo
                            solution.RemoveAssignment(receiver.UserId, nightShift.ShiftId, date3, force: true);
                            solution.RemoveAssignment(userY.UserId, nightShift.ShiftId, date2, force: true);
                            solution.RemoveAssignment(userX.UserId, nightShift.ShiftId, date1, force: true);

                            Rollback3DayWindow(solution, receiver.UserId, date3, recSnap);
                            Rollback3DayWindow(solution, userY.UserId, date3, ySnap3);
                            Rollback3DayWindow(solution, userY.UserId, date2, ySnap2);
                            Rollback3DayWindow(solution, userX.UserId, date2, xSnap2);
                            Rollback3DayWindow(solution, userX.UserId, date1, xSnap1);

                            solution.AddAssignment(userY.UserId, nightShift.ShiftId, date3, ShiftLabel.Night, false, isSkeleton: yWasSkeleton);
                            solution.AddAssignment(userX.UserId, nightShift.ShiftId, date2, ShiftLabel.Night, false, isSkeleton: xWasSkeleton);
                            solution.AddAssignment(donor.UserId, nightShift.ShiftId, date1, ShiftLabel.Night, false, isSkeleton: donorWasSkeleton);
                        }
                    }
                }
            }
        ...
        ShiftRequirement nightShift)
    {
        ...
        foreach (var (donor, night) in EnumerateSurplusNights(solution, constraints, excludeUserId: receiver.UserId))
        {
            ...
            var globalVisited = new HashSet<(DateTime Day, int GiverId)> { (initialDate, donor.UserId) };
        ...
        ShiftRequirement nightShift)
    {
        ...
        var openDates = AllCandidateDates(constraints, holidayOnly: false)
            .Where(d => constraints.UserConstraints.Any(u => u.IsActive && ShiftEligibilityResolver.MayEverTakeLabel(u, ShiftLabel.Night) && HasSpecialtyCapacity(solution, constraints, nightShift, d, u.SpecialtyId)))
            .ToList();
        ...
        foreach (var vacantDate in openDates)
        {
            ...
            {
                var recSnapshot = Snapshot3DayWindow(solution, receiver.UserId, vacantDate);
                ClearConflictingForNight(solution, constraints, receiver, vacantDate);
                ...
                if (CountNights(solution, receiver.UserId) > receiverBefore
                    ...
                    && !ShiftCoverageGuard.HasAnyOverCapacity(solution, constraints)
                    && !AdjacentShiftRestRules.HasForbiddenAdjacentPair(solution.GetUserAllAssignments(receiver.UserId), constraints.HardRules))
                {
                    return true;
                }
                solution.RemoveAssignment(receiver.UserId, nightShift.ShiftId, vacantDate, force: true);
                Rollback3DayWindow(solution, receiver.UserId, vacantDate, recSnapshot);
            }

            // ۲) گیرنده نمی‌تواند مستقیماً صندلی خالی را بگیرد؛ جستجوی کاربر واسط bridge (1-hop bridge)
            foreach (var bridge in constraints.UserConstraints.Where(u => u.IsActive && u.UserId != receiver.UserId))
            {
                ...
                foreach (var bridgeDate in bridgeNights)
                {
                    ...
                    if (!HasSpecialtyCapacity(solution, constraints, nightShift, vacantDate, bridge.SpecialtyId)
                        || !HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, bridgeDate, receiver.SpecialtyId, bridge.UserId))
                    {
                        continue;
                    }

                    if (!CanAcceptNightAfterClearing(solution, constraints, bridge, nightShift, vacantDate)
                        || !CanAcceptNightAfterClearing(solution, constraints, receiver, nightShift, bridgeDate))
                    {
                        continue;
                    }

                    var bridgeWasSkeleton = bridgeAssignment != null && bridgeAssignment.IsSkeleton;
                    var bridgeSnapshotVacant = Snapshot3DayWindow(solution, bridge.UserId, vacantDate);
                    var bridgeSnapshotBridge = Snapshot3DayWindow(solution, bridge.UserId, bridgeDate);
                    var recSnapshot = Snapshot3DayWindow(solution, receiver.UserId, bridgeDate);
                    ...
                    // Delta undo
                    solution.RemoveAssignment(receiver.UserId, nightShift.ShiftId, bridgeDate, force: true);
                    solution.RemoveAssignment(bridge.UserId, nightShift.ShiftId, vacantDate, force: true);
                    Rollback3DayWindow(solution, receiver.UserId, bridgeDate, recSnapshot);
                    Rollback3DayWindow(solution, bridge.UserId, vacantDate, bridgeSnapshotVacant);
                    Rollback3DayWindow(solution, bridge.UserId, bridgeDate, bridgeSnapshotBridge);
                    solution.AddAssignment(bridge.UserId, nightShift.ShiftId, bridgeDate, ShiftLabel.Night, false, isSkeleton: bridgeWasSkeleton);
                }
            }
        }
        ...
        bool allowCriticalDonor = false)
    {
        ...
        var receiverSpecialtyId = receiverUser?.SpecialtyId ?? 0;
        return solution.GetShiftAssignments(nightShift.ShiftId, date)
            ...
            .Where(x => x.Donor != null && HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, date, receiverSpecialtyId, x.Donor!.UserId))
    ...
    public static List<DateTime> GetFeasibleNightDates(ShiftConstraints constraints, UserConstraint user)
    {
        ...
        var allDates = AllCandidateDates(constraints, holidayOnly: false).ToList();
        var unavailDates = new HashSet<DateTime>(user.UnavailableDates.Select(d => d.Date));
        var unavailNightSlots = new HashSet<DateTime>(user.UnavailableShiftSlots
            .Where(s => s.ShiftLabel == ShiftLabel.Night)
            .Select(s => s.Date.Date));
        var approvedNonNightSlots = new HashSet<DateTime>(user.RequiredShiftSlots
            .Where(s => s.ShiftLabel != ShiftLabel.Night)
            .Select(s => s.Date.Date));
        var approvedNightDates = new HashSet<DateTime>(user.RequiredShiftSlots
            .Where(s => s.ShiftLabel == ShiftLabel.Night)
            .Select(s => s.Date.Date));
        ...
        for (int i = 0; i < allDates.Count; i++)
        {
            ...
            // Rest rules with approved requests:
            if (i > 0)
            {
                var prevDate = allDates[i - 1].Date;
                if (!allowNightAfterNight && approvedNightDates.Contains(prevDate))
                {
                    continue;
                }
            }

            if (i < allDates.Count - 1)
            {
                var nextDate = allDates[i + 1].Date;
                if (!allowNightAfterNight && approvedNightDates.Contains(nextDate))
                {
                    continue;
                }
                // Morning after night is strictly forbidden
                if (user.RequiredShiftSlots.Any(s => s.Date.Date == nextDate && s.ShiftLabel == ShiftLabel.Morning))
                {
                    continue;
                }
                // Evening after night check
                if (!allowEveningAfterNight && user.RequiredShiftSlots.Any(s => s.Date.Date == nextDate && s.ShiftLabel == ShiftLabel.Evening))
                {
                    continue;
                }
            }
    ...
    public static int CalculateNightSlack(ShiftConstraints constraints, UserConstraint user)
    {
        if (!user.ExactNightShiftCount.HasValue)
        {
            return int.MaxValue;
    ...
    /// <summary>
    /// برای کاربرانی که حاشیه مانورشان صفر است (Slack == 0 یعنی تعداد کل روزهای ممکن دقیقاً برابر سهمیه شب است)،
    /// تمام تاریخ‌های ممکن فوراً به عنوان شب پیش‌انتساب داده می‌شوند تا ظرفیت شب یا فاصله روزهای مجاور توسط دیگران پر نشود.
    /// </summary>
    public static void PreassignZeroSlackNights(ShiftSolution solution, ShiftConstraints constraints)
    {
        var nightShift = constraints.ShiftRequirements.FirstOrDefault(s => s.ShiftLabel == ShiftLabel.Night);
        if (nightShift == null)
        {
        ...
        ShiftConstraints constraints)
    {
        return constraints.UserConstraints
            ...
            .Select(u =>
            {
                ...
                var slack = CalculateNightSlack(constraints, u);
                return (User: u, HolidayDeficit: holidayDeficit, TotalDeficit: totalDeficit, Slack: slack);
            })
            .Where(x => x.HolidayDeficit > 0 || x.TotalDeficit > 0)
            .OrderBy(x => x.Slack)
            .ThenByDescending(x => x.HolidayDeficit)
        ...
        while (swapped < needed)
        {
            ...
            foreach (var holidayDate in holidayDates)
            {
                ...
                foreach (var occupant in occupants)
                {
                    ...
                    if (!HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, holidayDate, user.SpecialtyId, donor.UserId)
                        || !HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, weekdayDate, donor.SpecialtyId, user.UserId))
                    {
                        continue;
                    }
        ...
        while (claimed < needed)
        {
            ...
            foreach (var date in dates)
            {
                ...
                if (CanDonateNight(solution, constraints, bridge, occupant, forHolidayClaim: holidayOnly)
                    && HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, date, user.SpecialtyId, occupant.UserId))
                {
                    ClearConflictingForNight(solution, constraints, user, date);
        ...
        int minGap)
    {
        ...
        foreach (var (donor, surplusNight) in surplusDates)
        {
            ...
            if (!HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, date, user.SpecialtyId, surplusNight.UserId))
            {
                continue;
            }
        ...
        bool holidayClaim)
    {
        ...
        return occupants
            ...
            .Where(x => x.Donor != null
                        && HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, date, receiverSpecialtyId, x.Donor!.UserId)
                        && CanDonateNight(solution, constraints, x.Donor!, x.Assignment, forHolidayClaim: holidayClaim))
        ...
        DateTime date)
    {
        ...
        var canKeepSameDayMorning = maxPerDay >= 2
                                    ...
                                    && ShiftEligibilityResolver.MayTakeLabelOnDate(user, ShiftLabel.Morning, day);
        ...
        DateTime nightDate)
    {
        ...
        return solution.GetUserAllAssignments(user.UserId)
            .Where(a =>
            {
                ...
                if (a.Date.Date == day && a.ShiftLabel == ShiftLabel.Evening && !IsProtected(constraints, user.UserId, a))
                {
                    return false;
                }

                if (a.Date.Date == day && a.ShiftLabel == ShiftLabel.Morning && !canKeepSameDayMorning && !IsProtected(constraints, user.UserId, a))
                {
                    return false;
                }
    ...
    public static bool CanAcceptNightAfterClearing(
        ShiftSolution solution,
        ShiftConstraints constraints,
        UserConstraint user,
        ShiftRequirement nightShift,
        DateTime date)
    {
        var existingWithoutClearable = AssignmentsIgnoringClearableForNight(solution, constraints, user, date);

        if (AdjacentShiftRestRules.WouldConflict(
                existingWithoutClearable,
                date,
                ShiftLabel.Night,
                constraints))
        {
        ...
        var canKeepSameDayMorning = maxPerDay >= 2
                                    && ShiftEligibilityResolver.SupportsMorningNightCombo(user)
                                    && ShiftEligibilityResolver.MayTakeLabelOnDate(user, ShiftLabel.Morning, date);
        ...
        return ShiftEligibilityResolver.MayTakeLabelOnDate(user, ShiftLabel.Night, date)
        ...
        for (var iter = 0; iter < 24; iter++)
        {
            ...
            foreach (var night in nights)
            {
                foreach (var target in allDates)
                {
                    ...
                    var occupants = solution.GetShiftAssignments(nightShift.ShiftId, target)
                        .Where(a => !a.IsOnCall && a.UserId != user.UserId)
                        .ToList();
                    ...
                    foreach (var occupant in occupants)
                    {
                        ...
                        if (!HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, target, user.SpecialtyId, other.UserId)
                            || !HasSpecialtyCapacityIgnoring(solution, constraints, nightShift, night.Date.Date, other.SpecialtyId, user.UserId))
                        {
                            continue;
                        }
        ...
        int minGap)
    {
        ...
        if (other.UnavailableShiftSlots.Any(s => s.Date.Date == userFrom && s.ShiftLabel == ShiftLabel.Night) ||
            user.UnavailableShiftSlots.Any(s => s.Date.Date == userTo && s.ShiftLabel == ShiftLabel.Night))
        {
            return false;
        }
        ...
        var otherMinGap = ResolveNightSpacingGap(constraints, other);
        var otherOtherNights = GetNights(solution, other.UserId)
            .Where(a => a.Date.Date != userTo)
            .Select(a => a.Date.Date);
        if (otherOtherNights.Any(d => Math.Abs((d - userFrom).Days) <= otherMinGap))
        {
            return false;
        }
