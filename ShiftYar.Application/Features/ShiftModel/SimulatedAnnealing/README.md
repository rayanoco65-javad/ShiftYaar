...
### 1. محدودیت‌های کاربر
- **تاریخ‌های غیرقابل دسترس**: روزهایی که کاربر نمی‌تواند شیفت داشته باشد (شامل OFF تأییدشده و **شب روز قبل OFF صبح/کل‌روز**)
- **شیفت‌های غیرقابل دسترس (slot)**: عدم حضور در شیفت مشخص (`UnavailableShiftSlots`)
- **شیفت‌های ترجیحی**: شیفت‌هایی که کاربر ترجیح می‌دهد
- **شیفت‌های ناخواسته**: شیفت‌هایی که کاربر نمی‌خواهد
- **مجوزهای صریح نوع شیفت** (`UserShiftPermission` flags روی User): صبح، عصر، شب، صبح/عصر، صبح/شب
- **سقف موظفی بهره‌وری**: با `ProductivityRequiredHoursResolver` (خودکار یا `MaxProductivityRequiredHours` دستی)
- **اولویت پر کردن موظفی**: غیرطرحی قبل از طرحی؛ پرسنل طرحی (`IsProjectPersonnel`) فقط تا موظفی
...
- **سهمیه‌های روزانه ماهانه و تناوب هفتگی شیفت‌های روزانه (`IsWeeklyAlternatingActive` و `FirstWeekShiftLabel`)**: اعمال تناوب هفتگی صبح و عصر بر اساس تقویم استاندارد شمسی (شنبه تا جمعه)، با استثنای کامل شیفت شب و روزهای آف، و تقدم قطعی درخواست‌های شیفت تأییدشده پرسنل.
