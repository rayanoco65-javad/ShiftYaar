...
- **اولویت قطعی بر تناوب هفتگی:** درخواست شیفت تأییدشده بر قانون تناوب هفتگی اولویت دارد و در صورت تایید، شیفت درخواستی کاربر بدون مانع اعمال می‌گردد.

## 4.6) قانون تناوب هفتگی شیفت‌های روزانه (صبح و عصر)
- تنظیم از طریق سهمیه ماهانه روزانه (`UserMonthlyDayShiftQuota`):
  - `IsWeeklyAlternatingActive`: فعال‌سازی تناوب هفتگی برای پرسنل.
  - `FirstWeekShiftLabel`: تعیین شیفت هفته اول ماه شمسی (فقط `Morning` یا `Evening`).
- **منطق چرخش:** هفته‌های تقویمی استاندارد شمسی از شنبه تا جمعه محاسبه می‌شوند (هفته‌های ناقص ابتدا و انتهای ماه نیز پشتیبانی می‌شوند). هفته‌های فرد با شیفت اولیه و هفته‌های زوج با شیفت متضاد پر می‌شوند.
- **استثنائات:**
  1. **شیفت‌های شب، آف و مرخصی:** کاملاً از این قاعده مستثنی هستند و طبق روال عادی تخصیص می‌یابند.
  2. **درخواست‌های شیفت تأییدشده:** اگر برای روزی در هفته متضاد، درخواست تأییدشده وجود داشته باشد (مثلاً درخواست صبح در هفته‌ای که کاربر سهمیه عصر دارد)، آن درخواست تأییدشده به صورت استثنا در برنامه گنجانده می‌شود.
- **کنترل در موتور:** توسط `ShiftEligibilityResolver` فیلتر شده و در صورت انتساب غیرمجاز، توسط `ShiftEligibilityGuard` گزارش و پاکسازی می‌شود.
