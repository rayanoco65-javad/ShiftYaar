...
### فعال‌سازی
- `EnableOvertimeDistributionBySeniority`: فعال/غیرفعال کردن توزیع اضافه کار بر اساس سابقه (در صورت انتخاب مقدار ۰ یا ۱ در `OvertimePreferenceType`، این گزینه به صورت خودکار فعال (`true`) می‌شود).

### نوع تنظیمات اضافه کار
- `OvertimePreferenceType`: نوع ترجیح اضافه کار در دپارتمان
  - `0`: **علاقه‌مند به اضافه کار** - شیفت‌های مازاد بر موظفی به نسبت سابقه بین پرسنل متقاضی توزیع می‌شود (پرسنل با سابقه بیشتر اضافه کار بیشتری دریافت می‌کنند).
  - `1`: **گریزان از اضافه کار** - شیفت‌های مازاد بر موظفی به پرسنل با سابقه کمتر اختصاص می‌یابد تا پرسنل با سابقه بالاتر در حد موظفی (اضافه کار نزدیک به صفر) محافظت شوند.
  - `2`: **خنثی / توزیع برابر** - اضافه کار به طور مساوی بین پرسنل متقاضی تقسیم می‌شود.

### وزن و شیب تنظیمات
- `OvertimeDistributionWeight`: وزن اعمال ترجیح اضافه کار در الگوریتم بهینه‌سازی
- `OvertimeSeniorityDistributionSlope`: شیب اختصاصی تقسیم اضافه کار بر اساس سنوات ($0.1 \le \text{Slope} \le 10.0$ با پیش‌فرض $1.0$).
  - **نقش شیب:** تنظیم شدت اثر سابقه بر سهم اضافه کار.
    - مقادیر ملایم (مانند $0.5$): تفاوت سهم پرسنل با سوابق مختلف ملایم و با شیب کم تغییر می‌کند.
    - مقدار پیش‌فرض ($1.0$): رفتار خطی و متناسب استاندارد.
    - مقادیر تند (مانند $1.5$ تا $2.5$): ایجاد تفاوت شدید و حداکثری بین پرسنل باسابقه و کم‌سابقه.
  - در صورت خالی بودن (`null`)، از مقدار `SeniorityDistributionSlope` دپارتمان یا پیش‌فرض $1.0$ استفاده می‌شود.

> **نکته مهم در تعامل با رضایت کاربر:** این تنظیم با فیلد `OvertimeConsent` در پروفایل کاربر هماهنگ است. پرسنلی که `OvertimeConsent = false` دارند از اضافه‌کار معاف هستند، و توزیع بر اساس سابقه فقط میان پرسنل متقاضی (`OvertimeConsent = true`) انجام می‌شود.
...
### مثال 6: دپارتمان گریزان از اضافه کار با شیب تند
```json
{
  "EnableOvertimeDistributionBySeniority": true,
  "OvertimePreferenceType": 1,
  "OvertimeSeniorityDistributionSlope": 2.0,
  "OvertimeDistributionWeight": 2.5
}
```
