...
### 4.2) سهمیه شیفت روزانه و قانون تناوب هفتگی (`UserMonthlyDayShiftQuota`)
سوپروایزر می‌تواند سهمیه شیفت‌های روزانه (صبح و عصر) و همچنین قانون تناوب هفتگی را برای هر پرسنل در ماه شمسی تنظیم نماید.
- Endpoint تکی: `POST /UpsertUserMonthlyDayShiftQuota`
- Endpoint گروهی: `POST /UpsertDepartmentMonthlyDayShiftQuotas`
- فیلدهای سهمیه عددی:
  - `exactMorningShiftCount` / `morningFallbackParticipation`: سهمیه قطعی صبح و وضعیت مشارکت در مازاد
  - `exactHolidayMorningShiftCount` / `morningHolidayFallbackParticipation`: سهمیه صبح در روزهای تعطیل
  - `exactEveningShiftCount` / `eveningFallbackParticipation`: سهمیه قطعی عصر و وضعیت مشارکت در مازاد
  - `exactHolidayEveningShiftCount` / `eveningHolidayFallbackParticipation`: سهمیه عصر در روزهای تعطیل
- **قانون تخصیص شیفت‌های متناوب هفتگی (صبح و عصر):**
  - `isWeeklyAlternatingActive`: فعال‌سازی قانون تناوب هفتگی برای پرسنل (`true`/`false`).
  - `firstWeekShiftLabel`: شیفت آغازین هفته اول ماه شمسی (منحصراً `0` برای صبح یا `1` برای عصر).
  - **مبنای تقویمی:** هفته‌های تقویمی استاندارد ایران از **شنبه تا جمعه** محاسبه می‌شوند و هفته‌های ناقص در ابتدا یا انتهای ماه شمسی به عنوان هفته‌های تقویمی مستقل (هفته ۱، هفته ۲، ...) به درستی مدیریت می‌شوند.
  - **الگوی چرخش هفتگی:** در هفته‌های فرد (۱، ۳، ۵) شیفت اولیه مجاز است؛ در هفته‌های زوج (۲، ۴) شیفت متضاد روزانه (صبح ↔ عصر) مجاز است.
  - **استثنای مطلق شیفت شب و روزهای آف/مرخصی:** شیفت‌های شب، روزهای مرخصی و آف کاملاً از این قاعده مستثنی هستند و طبق روال عادی خود بدون هیچ محدودیتی تخصیص می‌یابند.
  - **استثنای قطعی و ارجحیت مطلق درخواست‌های شیفت تأییدشده:** اگر برای روزی در هفته متضاد، درخواست تأییدشده وجود داشته باشد (برای نمونه: اگر پرسنل در هفته دوم طبق تناوب موظف به عصر باشد، اما یک درخواست حضور صبح تأییدشده برای روزی در آن هفته داشته باشد)، درخواست تأییدشده بر قانون تناوب ارجحیت مطلق داشته و شیفت صبح بدون مانع در برنامه زمان‌بندی اعمال خواهد شد.

### 4.3) ثبت درخواست‌های شیفت توسط کارکنان
...
- **اولویت مطلق درخواست‌های شیفت تأییدشده بر سایر قوانین:**
  ...
  - **اولویت بر قانون تناوب هفتگی شیفت‌های روزانه:** در صورتی که برای کاربر قانون تناوب هفتگی فعال باشد (مثلاً در هفته دوم فقط شیفت عصر داشته باشد)، در صورت وجود یک درخواست شیفت تأییدشده برای شیفت متضاد (مثلاً صبح)، درخواست تأییدشده از قاعده تناوب مستثنا بوده و شیفت متضاد درخواستی در برنامه اعمال می‌شود.
...
### 4.6) مجوز نوع شیفت (`AllowedShiftPermissions`)
